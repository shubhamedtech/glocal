...
if ($student->num_rows > 0) {
    while ($row = $student->fetch_assoc()) {
        ...
        // echo "SELECT marksheets.obt_marks_ext, marksheets.obt_marks_int,marksheets.obt_marks,marksheets.status,marksheets.remarks,marksheets.Created_At,Syllabi.Code,Syllabi.Name as subject_name,Syllabi.Min_Marks, Syllabi.Max_Marks FROM marksheets LEFT JOIN Syllabi ON marksheets.subject_id = Syllabi.ID WHERE enrollment_no = '" . $data['Enrollment_No'] . "'  $sem_sql ORDER BY Syllabi.Code ASC";die;
        $temp_subjects = $conn->query("SELECT marksheets.exam_month,marksheets.exam_year, marksheets.obt_marks_ext, marksheets.obt_marks_int,marksheets.obt_marks,marksheets.status,marksheets.remarks,marksheets.Created_At,Syllabi.Code,Syllabi.Name as subject_name,Syllabi.Min_Marks, Syllabi.Max_Marks FROM marksheets LEFT JOIN Syllabi ON marksheets.subject_id = Syllabi.ID WHERE enrollment_no = '" . $data['Enrollment_No'] . "'  $sem_sql ORDER BY Syllabi.Code ASC");
        $data['marks'] = array();
        $temp_subject = [];
        $total_obt = 0;
        $total_max = 0;
        ...
        if ($temp_subjects->num_rows > 0) {
            while ($temp_subject = $temp_subjects->fetch_assoc()) {
                ...
                if ($temp_subject != null) {
                    ...
                    $exam_month = $temp_subject['exam_month'].'-'.$temp_subject['exam_year'];
                    ...
                    $o_int_marks[] = $obt_marks_int + $obt_marks_ext;
            ...
            if ($data['University_ID'] == 48) {
                ...
                $pdf->SetXY(15, 72.4);
                $pdf->Cell(0, 0, $data['duration_val'] . ' ' . 'in' . ' ' . ucwords(strtolower($data['course'])), 0, 0, 'C', 0);
                $pdf->SetXY(15, 81);
                ...
                foreach ($data['marks'] as $mark) {
                    ...
                    if (strlen($mark['subject_name']) > 30) {
                        ...
                        $pdf->Cell($cellWidth, $cellHeight - 10, $mark['obt_marks_int'] + $mark['obt_marks_ext'], 'LB', 0, 'C');
                        ...
                        $pdf->Cell($cellWidth, $cellHeight, $mark['obt_marks_int'] + $mark['obt_marks_ext'], 'LB', 0, 'C');
                ...
                if ($count >= 9) {
                    ...
                    if (in_array('FAIL', $remark_status)) {
 
                        $pdf->SetXY(116, 229.4);
                        $pdf->Cell(35, 8, "FAIL", 'TLR', 1, 'C', 0);
                        $pdf->SetXY(151, 229.4);
                        $pdf->Cell(42, 8, '', 'TR', 1, 'C', 0);
                    } else {
                        ...
                        $pdf->SetXY(151, 246.6);
                        $pdf->Cell(42, 8, number_format($percentage, 2) . "%", 'R', 1, 'C', 0);
                    }
                ...
                $pdf->Cell(70, 10, 'Exam Session : ' . ' ' . ucwords(strtolower($exam_month)), 1, 0, 'L', 0);
                ...
                foreach ($data['marks'] as $mark) {
                    ...
                    if (strlen($mark['subject_name']) > 30) {
                        ...
                        $pdf->Cell(14, 10, $mark['obt_marks_int'] + $mark['obt_marks_ext'], 'BL', 0, 'C');
                ...
                if (in_array('FAIL', $remark_statuss)) {
                    $pdf->SetXY(116, 240.4);
                    $pdf->Cell(35, 8, "FAIL", 'TLR', 1, 'C', 0);
                    $pdf->SetXY(151, 240.4);
                    $pdf->Cell(42, 8, '', 'TR', 1, 'C', 0);
