...
if (isset($_POST['ids']) && isset($_POST['center'])) {
  ...
  foreach ($ids as $id) {
    $counter = 1;
    $getDurationQuery = $conn->query("SELECT Duration FROM Students WHERE ID = $id");
    if($getDurationQuery->num_rows > 0){
      $durationArr = $getDurationQuery->fetch_assoc();
      $durationQuery = " AND Student_Ledgers.Duration ='".$durationArr['Duration']."'";
    }
