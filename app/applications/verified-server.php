...
while ($row = mysqli_fetch_assoc($empRecords)) {
  ...
  if($_SESSION['university_id'] == 47){
    $currentSessin = date('y');
    $studentSession = explode('-',$row['Adm_Session'])[1];
    if($currentSessin>$studentSession)
    {
      $enterEnrollmentNumber = true;
    }
    else
    {
      $enterEnrollmentNumber = false;
    }
  $data[] = array(
    ...
    'enterEnrollmentNumber' => $enterEnrollmentNumber,
  );
}else{
  $data[] = array(
    "Photo" => empty($row['Location']) ? '/assets/img/default-user.png' : $row['Location'],
