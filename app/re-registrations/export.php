...
$studentIds = array();
$examSession = $conn->query("SELECT Name, Admission_Session FROM Exam_Sessions WHERE ID = " . $_SESSION['active_rr_session_id']);
...
while ($row = $students->fetch_assoc()) {
  ...
  $createdAt = !empty($row['Created_At']) ? date("d/m/Y H:i", strtotime($row['Created_At']." +5 hour +30 minutes")) : '';
  $finalData[] = array($row['Status'], $examSessionName, $row['Unique_ID'], $row['First_Name'], $row['Enrollment_No'], $row['Admission_Session_ID'], $row['RRDuration'], $row['Amount'], $row['Course_ID'], $user['Code'], $user['Name'], $rm['Name'], $createdAt);
}
