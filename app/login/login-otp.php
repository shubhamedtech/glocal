  ...
  if ($exam_student_check->num_rows > 0) {
    ...
    if ($check->num_rows > 0) {
      ...
      if ($user_details['Status'] == 1) {
        ...
        $check = $conn->query("SELECT ID FROM OTP_Verifications WHERE User_ID = ".$_SESSION['ID']);
        if($check->num_rows>0){
          $conn->query("UPDATE OTP_Verifications SET OTP = '$otp' WHERE User_ID = ".$_SESSION['ID']);
        }else{
        ...
        curl_setopt_array($curl, array(
          ...
          CURLOPT_POSTFIELDS => array("number" => $_SESSION['Mobile'], "message" => $message, "session" => "otp"),
        ));
        ...
        if ($_SESSION['Role'] === 'Center') {
          echo json_encode(['status' => 200, 'message' => $response['message'], 'url' => '/dashboards/center-dashborad', 'number'=>$_SESSION['Mobile']]);
        } else {
          echo json_encode(['status' => 200, 'message' => $response['message'], 'url' => '/admissions/applications', 'number'=>$_SESSION['Mobile']]);
        }    
        }else{
            echo json_encode(['status'=>400, 'message'=>$response['message']]);
        }
      ...
      if ($student->num_rows > 0) {
        ...
        $_SESSION['verify'] = 1;
        echo json_encode(['status' => 200, 'role'=>'Student', 'message' => 'Welcome ' . implode(" ", $student_name), 'url' => '/dashboard']);
      } else {
