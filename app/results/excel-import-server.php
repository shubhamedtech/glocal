  ...
  if (in_array($_FILES["file"]["type"], $mimes)) {
    ...
    for ($i = 0; $i < $totalSheet; $i++) {
      ...
      foreach ($reader as $row) {
        ...
        $obt_ext_marks = mysqli_real_escape_string($conn, trim($row[4]));
        ...
        // ECHO "<PRE>";
        // ECHO "SELECT ID, Min_Marks,Max_Marks,University_ID FROM Syllabi WHERE University_ID = " . $_SESSION['university_id'] . " AND (Code LIKE '$subject_code') AND Semester = '" . $semester . "'  AND Course_ID = $course_id AND Sub_Course_ID = $sub_course_id"; 
        $subjects = $conn->query("SELECT ID, Min_Marks,Max_Marks,University_ID FROM Syllabi WHERE University_ID = " . $_SESSION['university_id'] . " AND (Code LIKE '$subject_code') AND Semester = '" . $semester . "'  AND Course_ID = $course_id AND Sub_Course_ID = $sub_course_id");
        ...
        if ($obt_ext_marks == 'AB' || $obt_int_marks == 'AB' || $obt_ext_marks == 0 || $obt_int_marks == 0) {
          $remarks = "Fail";
          $obt_ext_marks = ($obt_ext_marks == 'AB') ? 0 : $obt_ext_marks;
          $obt_int_marks = ($obt_int_marks == 'AB') ? 0 : $obt_int_marks;
        }
        ...
        if ($check->num_rows > 0) {
          // $export_data[] = array_merge($row, ['Result Already Uploaded!']);
          // continue;
          ...
          $update = $conn->query("UPDATE `marksheets` SET `obt_marks_ext` = " . $obt_ext_marks . ",  `obt_marks_int` = " . $obt_int_marks . ",   `obt_marks` = '" . $total . "',  `remarks` = '" . $remarks . "',  `exam_month` = '" . $exam_month . "', `exam_year` = " . $exam_year . ", `updated_at` = '" . $created_at . "'
              WHERE enrollment_no = '" . $enrollment . "' AND subject_id = '".$subject_ids."' AND paper_code= '" . $paper_code . "' ");
          if ($update) {
        ...
        // ECHO "<PRE>";
        // ECHO "INSERT INTO `marksheets`(`enrollment_no`, `subject_id`, `obt_marks_ext`, `obt_marks_int`, `obt_marks`, `remarks`, `status`, `exam_month`,`exam_year`,`created_at`,`paper_code`) VALUES ('" . $enrollment . "', " . $subject_ids . ", " . $obt_ext_marks . ", $obt_int_marks, '" . $total . "', '" . $remarks . "',1, '" . $exam_month . "', '$exam_year','$created_at','" . $paper_code . "')";
      }
    }
